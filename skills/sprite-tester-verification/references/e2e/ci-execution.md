...
有计划时，从 `Project setup` 复用已核实的命令；直接运行已有 spec 时，从项目配置读取，不为运行测试强建计划。首次执行或配置有变化时，按 [项目适配](project-conventions.md) 核对 runner、版本、project、筛选范围与 setup/teardown；确认目标案例被发现，零测试或跳过不算执行。
...
失败时进入 [Heal](heal.md)，修复后回到本步骤复验。检查已通过且没有新改动、失败或未解决风险时，不再无理由重跑。
...
使用 [测试报告模板](../../assets/templates/test-report.md)，记录每项命令、版本、环境、通过/失败/未运行/受阻及证据。总体判定仅覆盖实际检查范围，说明未覆盖部分和剩余风险。
